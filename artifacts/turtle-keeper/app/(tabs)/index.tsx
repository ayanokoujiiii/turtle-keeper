...
type Log = { id: string; type: string; title: string; turtle: string; turtleId?: string; detail: string; date: string; dateKey?: string };
...
type Store = { turtles: Turtle[]; care: Care[]; careHistory: Record<string, string[]>; reminders: Reminder[]; logs: Log[]; exams: HealthExam[]; memories: Memory[] };
...
const persianDateParts = (date: Date) => Object.fromEntries(new Intl.DateTimeFormat('en-US-u-ca-persian', { year: 'numeric', month: 'numeric', day: 'numeric' }).formatToParts(date).filter((part) => part.type !== 'literal').map((part) => [part.type, Number(part.value)])) as { year: number; month: number; day: number };
const persianDateLabel = (date: Date) => new Intl.DateTimeFormat('fa-IR-u-ca-persian', { day: 'numeric', month: 'long', year: 'numeric' }).format(date);
...
const initialStore: Store = {
  ...
  careHistory: { [localDateKey()]: ['feed'] },
  reminders: [{ id: 'r1', title: 'زمان غذا', turtle: 'لاک‌پشت من', time: '10:00', repeat: 'هر روز', active: true }],
  logs: [
    { id: '1', type: 'غذا', title: 'برگ قاصدک', turtle: 'لاک‌پشت من', turtleId: 'first-turtle', detail: 'یک مشت کوچک · ۱۰:۱۴', date: 'امروز', dateKey: localDateKey() },
    { id: '2', type: 'آب', title: 'آب تازه شد', turtle: 'لاک‌پشت من', turtleId: 'first-turtle', detail: '۱۸:۲۰', date: 'دیروز', dateKey: localDateKey(addDays(new Date(), -1)) },
  ],
  exams: [],
  memories: [],
};
...
const migrateStore = (value: Store): Store => {
  const care = (value.care || initialStore.care).filter((item) => item.id !== 'weight');
  const logs = (value.logs || initialStore.logs).map((log) => ({
    ...log,
    dateKey: log.dateKey || (log.date === 'دیروز' ? localDateKey(addDays(new Date(), -1)) : localDateKey()),
  }));
  return {
    ...initialStore,
    ...value,
    care,
    careHistory: value.careHistory || { [localDateKey()]: care.filter((item) => item.done).map((item) => item.id) },
    logs,
    exams: value.exams || [],
...
export default function HomeScreen() {
  ...
  const [turtleDraftPhoto, setTurtleDraftPhoto] = useState('');
  const [logTurtleIds, setLogTurtleIds] = useState<string[]>([]);
  ...
  const [calendarDateKey, setCalendarDateKey] = useState(localDateKey());
  ...
  const toggleCare = (id: string) => setStore((s) => {
    const care = s.care.map((x) => x.id === id ? { ...x, done: !x.done, time: !x.done ? new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : undefined } : x);
    return { ...s, care, careHistory: { ...s.careHistory, [localDateKey()]: care.filter((item) => item.done).map((item) => item.id) } };
  });
  ...
  const openTurtleEditor = (turtle?: Turtle) => {
    ...
    setTurtleDraftPhoto(turtle?.photo || '');
  ...
  const saveTurtle = () => {
    ...
    setSelected(turtle); setModal(null); setEditingTurtleId(null); setTurtleDraftPhoto(''); resetDraft();
  };
  ...
  const openLog = (kind: string, log?: Log) => {
    ...
    setLogTurtleIds(log ? [log.turtleId || store.turtles.find((turtle) => turtle.name === log.turtle)?.id || selected.id] : store.turtles.map((turtle) => turtle.id));
  ...
  const addLog = (type: string) => {
    ...
    const targets = editingLogId ? logTurtleIds.slice(0, 1) : logTurtleIds.length ? logTurtleIds : store.turtles.map((turtle) => turtle.id);
    if (!targets.length) return Alert.alert('لاکپشتی وجود ندارد', 'ابتدا حداقل یک لاکپشت اضافه کنید.');
    const nextLogs: Log[] = targets.map((turtleId) => {
      const turtle = store.turtles.find((item) => item.id === turtleId);
      return { id: editingLogId || uid(), type: existing?.type || type, title: draft.title.trim(), turtle: turtle?.name || selected.name, turtleId, detail: draft.detail.trim() || 'همین حالا ثبت شد', date: existing?.date || 'امروز', dateKey: existing?.dateKey || localDateKey() };
    });
    setStore((s) => ({ ...s, logs: editingLogId ? s.logs.map((log) => log.id === editingLogId ? { ...log, ...nextLogs[0] } : log) : [...nextLogs, ...s.logs] }));
    setDraft((d) => ({ ...d, title: '', detail: '' })); setEditingLogId(null); setLogTurtleIds([]); setModal(null);
  };
  ...
  const exportData = async () => {
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: 'application/json', dialogTitle: 'ذخیره یا ارسال پشتیبان Turtle Keeper', UTI: 'public.json' });
      } else {
  ...
  const saveGeminiKey = async () => { await AsyncStorage.multiSet([['turtle-keeper-gemini-key', geminiKey.trim()], ['turtle-keeper-gemini-model', geminiModel.trim() || 'gemini-2.5-flash']]); Alert.alert('ذخیره شد', 'کلید و مدل Gemini فقط روی همین دستگاه ذخیره شدند.'); };
  ...
  const aiContext = () => JSON.stringify({
    turtles: store.turtles.map((turtle) => ({ name: turtle.name, species: turtle.species, gender: turtle.gender, weight: turtle.weight, shellLength: turtle.shellLength, notes: turtle.notes, weightHistory: turtle.weightHistory?.slice(-5), measurementHistory: turtle.measurementHistory?.slice(-5).map((record) => ({ length: record.length, weight: record.weight, detail: record.detail, date: record.date })) })),
    recentActivities: store.logs.slice(0, 25).map((log) => ({ type: log.type, title: log.title, turtle: log.turtle, detail: log.detail, date: log.date })),
    medicalExams: store.exams.slice(0, 10).map((exam) => ({ turtleId: exam.turtleId, date: exam.date, overallStatus: exam.overallStatus, eyes: exam.eyes.status, nose: exam.nose.status, mouth: exam.mouth.status, legs: exam.legs.status, skin: exam.skin.status, shell: exam.shell.status, generalCondition: exam.generalCondition.status, userNotes: exam.userNotes })),
  }, null, 2);
  const callGemini = async (prompt: string, imageBase64?: string) => {
    const requested = geminiModel.trim() || 'gemini-2.5-flash';
    const models = Array.from(new Set([requested, 'gemini-2.5-flash', 'gemini-2.5-flash-lite']));
    let lastError = 'پاسخی از سرویس هوش مصنوعی دریافت نشد.';
    for (const model of models) {
      const parts: Array<Record<string, unknown>> = [{ text: prompt }];
      if (imageBase64) parts.unshift({ inlineData: { mimeType: 'image/jpeg', data: imageBase64 } });
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(geminiKey.trim())}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts }], generationConfig: { temperature: 0.35, maxOutputTokens: 700 } }) });
      const data = await response.json();
      if (response.ok) {
        const text = data?.candidates?.[0]?.content?.parts?.map((part: { text?: string }) => part.text || '').join('').trim();
        if (text) return text;
        lastError = 'سرویس Gemini پاسخ متنی برنگرداند.';
      } else {
  ...
  const askGuide = async () => {
    ...
    const prompt = `تو راهنمای مراقبت از لاک‌پشت‌های زمینی هستی. به فارسی پاسخ بده. برای ${selected.name} یک نکته کوتاه و کاربردی روزانه درباره نگهداری، تغذیه، سلامت یا تحرک بده. گونه و وزن و سابقه‌ی همه‌ی لاکپشت‌ها را در داده‌های زیر بخوان و پاسخ را بر اساس آن شخصی‌سازی کن. غذاهای ثبت‌شده امروز: ${todayFoods}. اگر غذایی مشکوک، سمی یا نامناسب بود، اول با عبارت «هشدار غذایی:» هشدار واضح بده؛ در غیر این صورت با «وضعیت غذا: مناسب به نظر می‌رسد، اما جایگزین دامپزشک نیست.» شروع کن. تشخیص قطعی پزشکی نده و در موارد جدی مراجعه به دامپزشک را توصیه کن.\n\nداده‌های ثبت‌شده:\n${aiContext()}`;
     ...
     try {
        ...
        setGuideText(`اتصال به راهنمای هوشمند برقرار نشد. ${error instanceof Error ? error.message : 'لطفاً کلید و مدل Gemini را در تنظیمات بررسی کنید.'}`);
  ...
  const analyzeHealth = async () => {
    ...
    try {
      const text = await callGemini(`به‌عنوان تحلیل‌گر مراقبت از لاکپشت‌های زمینی، داده‌های همه‌ی لاکپشت‌ها، وزن‌ها، اندازه‌ها، فعالیت‌ها و معاینه‌های پزشکی زیر را بررسی کن. به فارسی در چند bullet کوتاه بگو: ۱) روند وزن و اندازه، ۲) کارهای انجام‌شده و الگوهای مراقبت، ۳) مواردی که نیاز به پیگیری دارند، ۴) سه پیشنهاد عملی. اگر داده کافی نیست صادقانه بگو. تشخیص قطعی پزشکی نده و برای وضعیت نگران‌کننده دامپزشک را پیشنهاد کن.\n\n${aiContext()}`);
      setAnalysisText(text);
    } catch (error) {
      setAnalysisText(`تحلیل هوشمند انجام نشد. ${error instanceof Error ? error.message : 'لطفاً اتصال Gemini را در تنظیمات بررسی کنید.'}`);
  ...
  const Home = () => <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
    ...
    <Card style={{ backgroundColor: c.secondary }}><Text style={[styles.cardKicker, { color: c.primary }]}>ثبت گروهی</Text><Text style={[styles.careLabel, { color: c.foreground, marginTop: 6 }]}>همه‌ی لاکپشت‌ها فعال هستند</Text><Text style={[styles.careTime, { color: c.mutedForeground }]}>هنگام ثبت غذا یا فعالیت، می‌توانید یک یا چند لاکپشت را انتخاب کنید.</Text></Card>
    ...
    <Card>{store.care.map((item) => <Pressable key={item.id} onPress={() => press(() => toggleCare(item.id))} style={styles.careRow}><View style={[styles.careIcon, { backgroundColor: item.done ? '#DCEEE0' : c.secondary }]}><Feather name={item.icon} size={18} color={item.done ? c.primary : c.mutedForeground} /></View><View style={styles.flex}><Text style={[styles.careLabel, { color: c.foreground }, item.done && styles.strike]}>{item.label}</Text><Text style={[styles.careTime, { color: c.mutedForeground }]}>{item.done ? `انجام شد در ${item.time}` : 'امروز انجام شود'}</Text></View><View style={[styles.check, { borderColor: item.done ? c.primary : c.border, backgroundColor: item.done ? c.primary : 'transparent' }]}>{item.done && <Feather name="check" size={14} color="#FFFFFF" />}</View></Pressable>)}</Card>
     <View style={styles.sectionRow}><Text style={[styles.sectionTitle, { color: c.foreground }]}>ثبت سریع</Text><Text style={[styles.muted, { color: c.mutedForeground }]}>برای یک یا چند لاکپشت</Text></View>
    <View style={styles.quickGrid}><Button label="غذا" icon="coffee" onPress={() => openLog('غذا')} secondary /><Button label="آب" icon="droplet" onPress={() => openLog('آب')} secondary /><Button label="نور / UV" icon="sun" onPress={() => openLog('نور')} secondary /></View>
         <View style={styles.quickGrid}><Button label="ثبت وزن و اندازه" icon="activity" onPress={() => { resetDraft(); setModal('measurement'); }} secondary /><Button label="خاطرات من" icon="image" onPress={() => setTab('memories')} secondary /></View><View style={styles.quickGrid}><Button label="معاینه پزشکی" icon="clipboard" onPress={() => setTab('exams')} secondary /></View>
    ...
    <Card style={{ backgroundColor: '#EEF5E8' }}><View style={styles.rowBetween}><View style={styles.flex}><Text style={[styles.cardKicker, { color: c.primary }]}>راهنمای امروز</Text><Text style={[styles.logTitle, { color: c.foreground, marginTop: 6 }]}>نکات اختصاصی برای نگهداری بهتر</Text><Text style={[styles.careTime, { color: c.mutedForeground }]}>غذاهای امروز را هم بررسی می‌کنم.</Text></View><Pressable onPress={() => { setTab('guide'); askGuide(); }} style={[styles.guideButton, { backgroundColor: c.primary }]}><Feather name="book-open" size={19} color="#FFFFFF" /></Pressable></View></Card>
      <View style={styles.sectionRow}><Text style={[styles.sectionTitle, { color: c.foreground }]}>سوابق ثبت‌شده</Text><Text style={[styles.muted, { color: c.mutedForeground }]}>{filteredLogs.length} مورد از همه‌ی لاکپشت‌ها</Text></View>
   ...
   const Turtles = () => <ScrollView contentContainerStyle={styles.scroll}><Header title="لاک‌پشت‌های من" sub={`${store.turtles.length} همراه دوست‌داشتنی`} /><Button label="افزودن لاک‌پشت" icon="plus" onPress={() => openTurtleEditor()} /><View style={{ height: 14 }} />{store.turtles.map((t) => <Pressable key={t.id} onPress={() => setSelected(t)}><Card style={[styles.turtleCard, selected.id === t.id && { borderColor: c.primary, borderWidth: 2 }]}><View style={[styles.turtlePhoto, { backgroundColor: c.secondary }]}>{t.photo ? <Image source={{ uri: t.photo }} style={styles.image} /> : <Feather name="heart" size={28} color={c.primary} />}</View><View style={styles.flex}><Text style={[styles.turtleName, { color: c.foreground }]}>{t.name}</Text><Text style={[styles.sub, { color: c.mutedForeground }]}>{t.species}</Text><Text style={[styles.meta, { color: c.mutedForeground }]}>{t.weight} گرم  ·  طول لاک {t.shellLength} سانتی‌متر</Text></View><Pressable onPress={() => openTurtleEditor(t)} hitSlop={10}><Feather name="edit-2" size={18} color={c.primary} /></Pressable><Pressable onPress={() => deleteTurtle(t)} hitSlop={10}><Feather name="trash-2" size={18} color="#C66A62" /></Pressable></Card></Pressable>)}</ScrollView>;
    const Calendar = () => {
      const now = new Date();
      const currentParts = persianDateParts(now);
      const monthStart = addDays(now, 1 - currentParts.day);
      const monthDates: Date[] = [];
      for (let date = monthStart; persianDateParts(date).month === currentParts.month && monthDates.length < 31; date = addDays(date, 1)) monthDates.push(date);
      const selectedDate = new Date(`${calendarDateKey}T12:00:00`);
      const selectedParts = persianDateParts(selectedDate);
      const selectedLogs = store.logs.filter((log) => (log.dateKey || localDateKey()) === calendarDateKey);
      const selectedCare = store.careHistory[calendarDateKey] || (calendarDateKey === localDateKey() ? store.care.filter((item) => item.done).map((item) => item.id) : []);
      const selectedLabel = persianDateLabel(selectedDate);
      return <ScrollView contentContainerStyle={styles.scroll}><Header title="تقویم مراقبت" sub="روی هر روز بزنید تا همان روز را ببینید" /><Card><View style={styles.rowBetween}><Text style={[styles.month, { color: c.foreground }]}>{new Intl.DateTimeFormat('fa-IR-u-ca-persian', { month: 'long', year: 'numeric' }).format(now)}</Text><Text style={[styles.careTime, { color: c.mutedForeground }]}>{selectedParts.month === currentParts.month ? 'ماه جاری' : selectedLabel}</Text></View><View style={styles.week}><Text style={styles.day}>ش</Text><Text style={styles.day}>ی</Text><Text style={styles.day}>د</Text><Text style={styles.day}>س</Text><Text style={styles.day}>چ</Text><Text style={styles.day}>پ</Text><Text style={styles.day}>ج</Text></View><View style={styles.calendarGrid}>{Array.from({ length: (monthStart.getDay() + 1) % 7 }, (_, i) => <View key={`empty-${i}`} style={styles.date} />)}{monthDates.map((date) => { const keyForDate = localDateKey(date); const parts = persianDateParts(date); const isSelected = keyForDate === calendarDateKey; const isToday = keyForDate === localDateKey(); const hasLogs = store.logs.some((log) => (log.dateKey || localDateKey()) === keyForDate); return <Pressable key={keyForDate} onPress={() => setCalendarDateKey(keyForDate)} style={[styles.date, isSelected && { backgroundColor: c.primary }]}><Text style={{ color: isSelected ? '#fff' : c.foreground, fontWeight: isToday ? '700' : '400' }}>{new Intl.NumberFormat('fa-IR').format(parts.day)}</Text>{hasLogs && <View style={[styles.dateDot, { backgroundColor: isSelected ? '#FFFFFF' : c.accent }]} />}</Pressable>; })}</View></Card><Text style={[styles.sectionTitle, { color: c.foreground, marginTop: 22 }]}>{calendarDateKey === localDateKey() ? 'امروز' : selectedLabel}</Text><Card>{store.care.map((item) => { const done = selectedCare.includes(item.id); return <View key={item.id} style={styles.logRow}><Feather name={done ? 'check-circle' : 'circle'} size={20} color={done ? c.primary : c.mutedForeground} /><Text style={[styles.careLabel, { color: c.foreground, marginLeft: 12 }]}>{item.label}</Text></View>; })}{selectedLogs.length ? selectedLogs.map((log) => <View key={log.id} style={styles.logRow}><View style={[styles.dot, { backgroundColor: log.type === 'غذا' ? '#E8B85E' : '#82BBD0' }]} /><View style={styles.flex}><Text style={[styles.logTitle, { color: c.foreground }]}>{log.title}</Text><Text style={[styles.careTime, { color: c.mutedForeground }]}>{log.type} · {log.turtle} · {log.detail}</Text></View></View>) : <Text style={[styles.careTime, { color: c.mutedForeground, marginTop: 10 }]}>{calendarDateKey === localDateKey() ? 'هنوز فعالیتی برای امروز ثبت نشده است.' : 'برای این روز فعالیتی ثبت نشده است.'}</Text>}</Card></ScrollView>;
    };
  ...
  const screen = tab === 'home' ? <Home /> : tab === 'turtles' ? <Turtles /> : tab === 'calendar' ? <Calendar /> : tab === 'reminders' ? <Reminders /> : tab === 'stats' ? <VisualStats store={store} selected={selected} setSelected={setSelected} c={c} onEditMeasurement={openMeasurementEditor} onDeleteMeasurement={confirmDeleteMeasurement} onAnalyze={analyzeHealth} analysisText={analysisText} analysisLoading={analysisLoading} /> : tab === 'guide' ? <Guide /> : tab === 'memories' ? <Memories /> : tab === 'exams' ? <ExamArchive /> : <Settings />;
...
function VisualStats({ store, selected, setSelected, c, onEditMeasurement, onDeleteMeasurement, onAnalyze, analysisText, analysisLoading }: { store: Store; selected: Turtle; setSelected: (turtle: Turtle) => void; c: typeof colors.light | typeof colors.dark; onEditMeasurement: (record: MeasurementRecord) => void; onDeleteMeasurement: (record: MeasurementRecord) => void; onAnalyze: () => void; analysisText: string; analysisLoading: boolean }) {
  ...
  return <ScrollView contentContainerStyle={styles.scroll}><HeaderForStats title="گزارش سلامت و رشد" sub={`نمای کلی وضعیت ${selected.name}`} c={c} /><ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>{store.turtles.map((turtle) => <Pressable key={turtle.id} onPress={() => setSelected(turtle)} style={[styles.turtleChip, { backgroundColor: selected.id === turtle.id ? c.primary : c.secondary, borderColor: selected.id === turtle.id ? c.primary : c.border }]}><Feather name="heart" size={14} color={selected.id === turtle.id ? '#FFFFFF' : c.primary} /><Text style={{ color: selected.id === turtle.id ? '#FFFFFF' : c.foreground, fontWeight: '700', fontSize: 13 }}>{turtle.name}</Text></Pressable>)}</ScrollView>
    ...
    <Text style={[styles.sectionTitle, { color: c.foreground, marginTop: 22 }]}>تحلیل هوشمند همه‌ی لاکپشت‌ها</Text><CardForStats c={c}><Text style={[styles.careTime, { color: c.mutedForeground, marginBottom: 12 }]}>تحلیل بر اساس وزن، اندازه، فعالیت‌ها و معاینه‌های پزشکی ثبت‌شده انجام می‌شود.</Text><ButtonForStats label={analysisLoading ? 'در حال تحلیل...' : 'تحلیل وضعیت با Gemini'} icon="activity" onPress={onAnalyze} disabled={analysisLoading} c={c} />{analysisLoading ? <ActivityIndicator color={c.primary} style={{ marginTop: 18 }} /> : analysisText ? <Text style={[styles.guideText, { color: c.foreground, marginTop: 16 }]}>{analysisText}</Text> : null}</CardForStats>
  </ScrollView>;
}
...
function ButtonForStats({ label, icon, onPress, disabled, c }: { label: string; icon: keyof typeof Feather.glyphMap; onPress: () => void; disabled?: boolean; c: typeof colors.light | typeof colors.dark }) { return <Pressable onPress={onPress} disabled={disabled} style={[styles.button, { backgroundColor: disabled ? c.border : c.primary }]}><Feather name={icon} size={17} color={disabled ? c.mutedForeground : c.primaryForeground} /><Text style={[styles.buttonText, { color: disabled ? c.mutedForeground : c.primaryForeground }]}>{label}</Text></Pressable>; }
...
const styles: any = StyleSheet.create({
  turtleChip: { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderRadius: 18, paddingHorizontal: 13, paddingVertical: 9, marginRight: 8 }, selectionRow: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 8 }, filterChip: { flex: 1, minHeight: 38, borderWidth: 1, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
